...
export type ProjectTask = { id: string; projectId: string; code: string; title: string; assigneeName: string; startDate: string; dueDate: string; status: "not_started" | "in_progress" | "completed"; progressPercent: number };
export type Project = { id: string; code: string; name: string; managerName: string; startDate: string; endDate: string; budgetAmount: number; actualCost: number; actualRevenue: number; status: "not_started" | "in_progress" | "completed"; progressPercent: number };
export type ProjectOverview = { projects: Project[]; tasks: ProjectTask[]; employees: HrEmployee[] };
...
function projectOverview(): ProjectOverview {
  ...
  const tasks = ((data.projects?.tasks ?? []) as any[]).map((task) => projectTaskOverrides.get(task.id) ?? ({ id: task.id, projectId: task.projectId, code: task.code, title: task.title, assigneeName: nameFor(task.assigneeId), startDate: task.startDate, dueDate: task.dueDate, status: task.status, progressPercent: task.progressPercent }));
  const projects: Project[] = ((data.projects?.projects ?? []) as any[]).map((project) => { const projectTasks = tasks.filter((task) => task.projectId === project.id); const progressPercent = projectTasks.length ? Math.round(projectTasks.reduce((total, task) => total + task.progressPercent, 0) / projectTasks.length) : project.progressPercent; const status = projectTasks.length ? (projectTasks.every((task) => task.status === "completed") ? "completed" : projectTasks.every((task) => task.status === "not_started") ? "not_started" : "in_progress") : project.status; return { id: project.id, code: project.code, name: project.name, managerName: nameFor(project.managerId), startDate: project.startDate, endDate: project.endDate, budgetAmount: project.budgetAmount, actualCost: project.actualCost, actualRevenue: project.actualRevenue, status, progressPercent }; }); return { projects, tasks, employees };
}
...
const mockApiBase = {
  ...
  projects: {
    getOverview: () => respond<ProjectOverview>(projectOverview()),
    updateTask: (id: string, patch: Pick<ProjectTask, "status" | "progressPercent">) => { const task = projectOverview().tasks.find((item) => item.id === id); const errors: Record<string, string> = {}; if (!task) errors.form = "Không tìm thấy nhiệm vụ."; if (!Number.isFinite(patch.progressPercent) || patch.progressPercent < 0 || patch.progressPercent > 100) errors.progressPercent = "Tiến độ phải từ 0 đến 100%."; if (patch.status === "completed" && patch.progressPercent !== 100) errors.progressPercent = "Nhiệm vụ hoàn thành phải có tiến độ 100%."; if (Object.keys(errors).length || !task) return respond(mutation<ProjectTask>(null, errors)); const entity = { ...task, ...patch }; projectTaskOverrides.set(id, entity); return respond(mutation(entity)); },
  },
  manufacturing: {
...
const mutationPermissions: Record<string, PermissionRule> = {
  ...
  "projects.updateTask": { module: "Dự án", action: "Sửa" },
